...
class CsobClientTests(TestCase):
    ...
    def test_payment_init_success(self):
        ...
        responses.add(
            responses.POST, resp_url, body=json.dumps(resp_payload), status=200
        )
    ...
    def test_payment_init_bad_cart(self):
        ...
        resp_payload = utils.mk_payload(
            ...
            pairs=(
                ...
                (
                    "resultMessage",
                    "Invalid 'cart' amounts, does not sum to totalAmount",
                ),
    ...
    def test_payment_status_extension(self):
        ...
        responses.add(
            responses.GET, resp_url, body=json.dumps(resp_payload), status=200
        )
    ...
    def test_gateway_return_merchant_data(self):
        ...
        self.assertEqual(
            r,
            OrderedDict(
                [("resultCode", 110), ("paymentStatus", 7), ("merchantData", b"Foo")]
            ),
        )

    @staticmethod
    def test_get_card_provider():
    ...
    def test_payment_init_with_too_long_merchant_data(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Merchant data length encoded to BASE64 is over 255 chars"
        ):
            self.c.payment_init(
                order_no=666,
                total_amount="66600",
    ...
    def test_description_strip(self):
        ...
        with patch(
            "pycsob.utils.mk_payload", return_value=resp_payload
        ) as mock_mk_payload:
            self.c.payment_init(
                42, "100", "http://example.com", "Konference Internet a Technologie 19"
            )

        self.assertEqual(
            mock_mk_payload.mock_calls,
            [
                call(
                    KEY_PATH,
                    pairs=(
